...
def sideplankf(whattype,till,camera):
    pose_video = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1)
    camera_video = cv2.VideoCapture(int(camera))
    camera_video.set(3,1280)
    camera_video.set(4,960)
    cv2.namedWindow('SIDE PLANK', cv2.WINDOW_NORMAL)
    count=0
    up=0
    down=0
    rightnow="down"
    ...
    while camera_video.isOpened():
        ...
        if landmarks:
            ...
            cv2.putText(frame, 'SIDE PLANK', (65,30),
            ...
            if angle1>=170 and angle4>=170  and angle5>=160 and angle8>=160 and angle2<=290 and angle3>=70:
                cv2.rectangle(frame, (0, 0), (225, 73), correct,-1)
                decide=1
            else:
                cv2.rectangle(frame, (0, 0), (225, 73), wrong,-1)
                decide=0
