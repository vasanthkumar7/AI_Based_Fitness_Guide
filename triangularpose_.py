...
def triangularf(whattype,till,camera):
    pose_video = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1)
    camera_video = cv2.VideoCapture(int(camera))
    camera_video.set(3,1280)
    camera_video.set(4,960)
    cv2.namedWindow('TRIANGULAR POSE', cv2.WINDOW_NORMAL)
    count=0
    up=0
    down=0
    rightnow="down"
    ...
    while camera_video.isOpened():
        ...
        if landmarks:
            ...
            cv2.putText(frame, 'TRIANGULAR POSE', (65,30),
            ...
            if angle1>=170 and angle9>=170 and angle10>=170 and angle5>=270:
