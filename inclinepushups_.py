...
def inclinepushupsf(whattype,till,camera):
    pose_video = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1)
    camera_video = cv2.VideoCapture(int(camera))
    camera_video.set(3,1280)
    camera_video.set(4,960)
    cv2.namedWindow('INCLINE PUSHUPS', cv2.WINDOW_NORMAL)
    count=0
    up=0
    down=0
    rightnow="down"
    ...
    while camera_video.isOpened():
        ...
        if landmarks:
            ...
            cv2.putText(frame, 'INCLINE PUSHUPS', (65,30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

            


            if angle1>=200 and angle4>=20   and rightnow=="down":
                up+=1
                rightnow="up"
            if angle1<=190 and angle4<=200    and rightnow=="up":
                rightnow="down"
                down+=1
                count+=1
        ...
        cv2.imshow('INCLINE PUSHUPS', frame)
