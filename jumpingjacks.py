...
def jumpingJacks(whattype,till,camera):
    pose_video = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1)
    camera_video = cv2.VideoCapture(int(camera))
    camera_video.set(3,1280)
    camera_video.set(4,960)
    cv2.namedWindow('JUMPING JACKS', cv2.WINDOW_NORMAL)
    count=0
    up=0
    down=0
    rightnow="down"
    ...
    while camera_video.isOpened():
        ...
        if landmarks:
            ...
            cv2.putText(frame, 'JUMPING JACKS', (65,30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)


            if angle2<=235 and angle3>=125 and angle6>=100 and angle7<=260 and rightnow=="down":
                rightnow="up"
                up+=1
            if angle2>=340 and angle3<=20 and angle6<=95 and angle7>=270 and rightnow=="up":
                rightnow="down"
                down+=1
                count+=1
        ...
        cv2.imshow('JUMPING JACKS', frame)
