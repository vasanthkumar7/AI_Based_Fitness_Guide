...
def sideleglift_rightf(whattype,till,camera):
    pose_video = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1)
    camera_video = cv2.VideoCapture(int(camera))
    camera_video.set(3,1280)
    camera_video.set(4,960)
    cv2.namedWindow('SIDE LEG LIFT right', cv2.WINDOW_NORMAL)
    count=0
    up=0
    down=0
    rightnow="down"
    ...
    while camera_video.isOpened():
        ...
        if landmarks:
            ...
            angle3 = calculateAngle(landmarks[mp_pose.PoseLandmark.right_ELBOW.value],landmarks[mp_pose.PoseLandmark.right_SHOULDER.value],landmarks[mp_pose.PoseLandmark.right_HIP.value])
            angle4 = calculateAngle(landmarks[mp_pose.PoseLandmark.right_SHOULDER.value],landmarks[mp_pose.PoseLandmark.right_ELBOW.value],landmarks[mp_pose.PoseLandmark.right_WRIST.value])
            ...
            angle6 = calculateAngle(landmarks[mp_pose.PoseLandmark.right_HIP.value], landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value],landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value])
            angle7 = calculateAngle(landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value], landmarks[mp_pose.PoseLandmark.right_HIP.value],landmarks[mp_pose.PoseLandmark.right_KNEE.value])
            ...
            angle10 = calculateAngle(landmarks[mp_pose.PoseLandmark.right_HIP.value], landmarks[mp_pose.PoseLandmark.right_KNEE.value],landmarks[mp_pose.PoseLandmark.right_ANKLE.value])
            ...
            cv2.putText(frame, 'SIDE LEG LIFT right', (65,30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

            


            if angle7<=220  and rightnow=="down":
                up+=1
                rightnow="up"
            if angle7>=240 and rightnow=="up":
                rightnow="down"
                down+=1
                count+=1
        ...
        cv2.imshow('SIDE LEG LIFT right', frame)
